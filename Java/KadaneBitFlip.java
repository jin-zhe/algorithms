...
/**
 * URL: http://www.careercup.com/question?id=6262507668766720
 */
public class KadaneBitFlip {
	int[] arr;
	boolean isAllOnes; // corner case: all zeroes
	
	KadaneBitFlip(int[] arr, boolean isAllOnes) {
		this.arr = arr;
		this.isAllOnes = isAllOnes;
	}
	
	/**
	 * run modified Kadane's algorithm
	 * O(N)
	 * @return	returns optimal range (start, end) to flip
	 */
	Pair<Integer,Integer> solve() {
		if (isAllOnes) {
			return new Pair<Integer, Integer>(0, arr.length);	// corner case
		}
		int temp_start = -1;
...
//		System.out.println(startEnd);	// check
	...
	public static void main(String[] args) {
		...
		KadaneBitFlip sol = new KadaneBitFlip(arr, isAllOnes);
		sol.run();
		sc.close();
	}
